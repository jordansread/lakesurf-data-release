...
# check the file name in case you aren't familiar with what sprintf() does:
predict_fl
```

    ## [1] "tmp/02_predicted_temp_N40-53_W67-98.nc"
...
Now set up data for a plot, and then plot July 1st temperatures
estimated for these lakes using ggplot2:

``` r
plot_data <- tibble(surftemp = all_surftemp, lat = lake_lat, lon = lake_lon) %>% 
